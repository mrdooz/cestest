...
static FILE* outputs[] = { 0, stdout, stdout, stderr, stderr };
...
#ifdef _WIN32
    char buf[1024];
    sprintf(buf, fmt, entry.file, entry.line);
    OutputDebugStringA(buf);
#else
    fprintf(outputs[entry.level], fmt, entry.file, entry.line);
#endif
  }
